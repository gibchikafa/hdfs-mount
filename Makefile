...
HOSTNAME=`hostname`
all: hdfs-mount 
hdfs-mount: *.go $(GOPATH)/src/bazil.org/fuse $(GOPATH)/src/github.com/gibchikafa/hdfs $(GOPATH)/src/golang.org/x/net/context $(GOPATH)/src/github.com/golang/protobuf/proto $(GOPATH)/src/golang.org/x/sys/unix
...
$(GOPATH)/src/github.com/gibchikafa/hdfs:
	[ -f submodules/gibchikafa-hdfs/.git ] || { echo "ERROR: Please clone hdfs-mount repository recursively: git clone http://github.com/Microsoft/hdfs-mount --recursive"; exit 1; }
	mkdir -p $(shell dirname $@)
	ln -s ../../../../submodules/gibchikafa-hdfs $@
